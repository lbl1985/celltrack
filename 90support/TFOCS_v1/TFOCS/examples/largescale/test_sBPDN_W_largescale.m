...
this requires PsiTransposeWFF.m and PsiWFF.m (should be included in this directory)
...
PLAY = input('Play the noisy music clip? ','s');
if strcmpi(PLAY,'y') || strcmpi(PLAY,'yes')
    sound( x_noisy );
end
...
%     weightsMatrix   = linop_matrix(weightsMatrix);
...
%%
PLAY = input('Play the recovered signal music clip? ','s');
if strcmpi(PLAY,'y') || strcmpi(PLAY,'yes')
    sound( x );
end
PLAY = input('Play the noisy music clip? ','s');
